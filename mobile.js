...
import {
    ...
    persistState, ZOOM_MIN, ZOOM_MAX, setTool,
} from './main.js';
...
function bindBarButtons() {
    ...
    const mPanBtn = $('mPanBtn');
    const mBrushBtn = bar.querySelector('.m-btn[data-target="toolBrush"]');
    const updateToolButtons = () => {
        const tool = state.tool;
        if (mPanBtn) {
            const isPan = tool === 'pan';
            mPanBtn.classList.toggle('active', isPan);
            mPanBtn.setAttribute('aria-pressed', isPan ? 'true' : 'false');
        }
        if (mBrushBtn) {
            const isBrush = tool === 'brush';
            mBrushBtn.classList.toggle('active', isBrush);
            mBrushBtn.setAttribute('aria-pressed', isBrush ? 'true' : 'false');
        }
    };

    if (mPanBtn) {
        mPanBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            setTool('pan');
            mPanBtn.classList.add('active');
            mPanBtn.setAttribute('aria-pressed', 'true');
            if (mBrushBtn) {
                mBrushBtn.classList.remove('active');
                mBrushBtn.setAttribute('aria-pressed', 'false');
            }
        });
    }

    if (mBrushBtn) {
        mBrushBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            setTool('brush');
            mBrushBtn.classList.add('active');
            mBrushBtn.setAttribute('aria-pressed', 'true');
            if (mPanBtn) {
                mPanBtn.classList.remove('active');
                mPanBtn.setAttribute('aria-pressed', 'false');
            }
        });
    }
