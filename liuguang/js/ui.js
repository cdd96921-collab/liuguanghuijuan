...
autoBtn.innerHTML = iconHTML('play') + '<span>自动</span>';
preserveBtn.innerHTML = iconHTML('pin') + '<span>保留</span>';
audioBtn.innerHTML = iconHTML('music') + '<span>音频</span>';
undoBtn.innerHTML = iconHTML('undo') + '<span>撤销</span>';
redoBtn.innerHTML = iconHTML('redo') + '<span>重做</span>';
if (shapeBtn) shapeBtn.innerHTML = iconHTML('circle') + '<span>圆形</span>';
bgToggle.innerHTML = iconHTML('moon') + '<span>背景</span>';
...
clearBtn.innerHTML = iconHTML('clear') + '<span>清空</span>';
settingsBtn.innerHTML = iconHTML('settings') + '<span>设置</span>';
galleryBtn.innerHTML = iconHTML('gallery') + '<span>作品库</span>';
...
diaryBtn.innerHTML = iconHTML('diary') + '<span>日记</span>';
saveBtn.innerHTML = iconHTML('save') + '<span>保存</span>';
if (layerBtn) layerBtn.innerHTML = iconHTML('layers') + '<span>图层</span>';
$('galleryClose').innerHTML = iconHTML('close');
...
export function setBgDarkUI(dark) {
    ...
    if (dark) { wrapper.style.background = THEMES[state.theme].bg; bgToggle.innerHTML = iconHTML('moon') + '<span>背景</span>'; }
    else { wrapper.style.background = LIGHT_BG; bgToggle.innerHTML = iconHTML('sun') + '<span>背景</span>'; }
}
...
export function positionMenu(menu, button) {
    ...
    const left = sidebarCollapsed ? 20 : btnRect.right + 8;
...
export function toggleParamsMenu() {
    if (!paramsMenu || !paramsBtn) return;
...
export function initUI() {
    ...
    // FPS 监控点击隐藏
    fpsMonitor.addEventListener('click', () => { fpsMonitor.classList.add('hidden'); });
    ...
    // 日记关闭
    $('diaryClose').onclick = closeDiary;
    diaryOverlay.onclick = closeDiary;
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!audioStyleMenu.contains(e.target) && e.target !== audioBtn) audioStyleMenu.classList.remove('show');
        if (paramsMenu && !paramsMenu.contains(e.target) && e.target !== paramsBtn) closeParamsMenu();
    });
