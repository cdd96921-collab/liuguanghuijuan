...
/** 运行时判定是否为移动端（响应 resize/旋转）。 */
export function getIsMobile() { return matchMedia('(max-width: 860px)').matches; }
/** 运行时判定是否为移动端横屏。 */
export function getIsLandscapeMobile() { return getIsMobile() && matchMedia('(orientation: landscape)').matches; }
...
export class Particle {
    ...
    draw(ctx) {
        ...
        if (state.shape === 'circle') {
            ctx.fillStyle = `hsl(${Math.round(this.h)}, ${Math.round(this.s)}%, ${Math.round(this.l)}%)`;
            ctx.beginPath();
            ctx.arc(this.x, this.y, Math.max(radius, 2), 0, Math.PI * 2);
            ctx.fill();
        } else {
            const sprite = getSprite(state.theme, this.h);
            if (sprite) {
...
export function draw(now) {
    ...
    const layerCtx = activeLayer ? activeLayer.ctx : ctx;
...
export function spawnInitialDust(count) {
    for (let i = 0; i < count; i++) {
        ...
        const p = acquire().reset(rand(0, W), rand(0, H), rand(8.0, 15.0), speed, state.theme);
        p.life = rand(0.2, 0.6);
